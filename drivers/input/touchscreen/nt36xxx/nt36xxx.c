...
static void nvt_ts_work_func(struct work_struct *work)
{
	...
	ret = nvt_rmi4_i2c_write_byte(ts->client, 0xff, 0x00 );
	ret = nvt_rmi4_i2c_read_word(ts->client, F01_RMI_DATA_BASE);
	...
	//TPD_ERR("%s status[0x%x],inte[0x%x]\n",__func__,status,inte);
		...
		status_check = nvt_init_panel(ts);
		if (status_check < 0) {
			TPD_ERR("nvt_init_panel failed\n");
		}
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (gesture != DouTap)
		nvt_get_coordinate_point(ts);

    TPD_DEBUG("gesture suport DouTap:%d\n",\
        DouTap_gesture);
...
static int nvt_init_panel(struct nvt_ts_data *ts)
{
	int ret;
    ...
    ret = nvt_mode_change(0x80);//change tp to doze mode
	if( ret < 0 ){
		msleep(150);
        ret = nvt_mode_change(0x80);
		if( ret < 0 ){
